  ...
  public:
    ...
    find(int p)
    {
      ...
      for (int i = 0; i < size; i++)
      {
        ...
        MPI_Improbe(i, find_parent_tag + i, MPI_COMM_WORLD, &find_parent_flag,
                    &parent_handle, &parent_status);
        ...
        if (find_state_flag)
        {
          ...
          MPI_Mrecv(&state_message, 1, MPI_INT, &state_handle, &state_status);
          state = state_message;
          this->validate(state);
      ...
      if (p % size != rank)
      {
        ...
        while (!find_rsp_state_flag)
        {
          ...
          MPI_COMM_WORLD,
                      &find_rsp_state_flag, &state_rsp_handle,
                      &state_rsp_status);
