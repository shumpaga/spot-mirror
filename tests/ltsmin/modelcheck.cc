  ...
  switch (key)
    {
      ...
      case PSEUDO_DISTRIBUTED:
        ...
        mc_options.nb_processes = to_int(arg, "--distributed");
        break;
...
const struct argp finput_argp = {options, parse_opt_finput, nullptr, nullptr,
...
static int checked_main()
{
  ...
  if (mc_options.distributed && mc_options.pseudo_distributed)
    {
      mc_options.distributed = true;
      mc_options.pseudo_distributed = false;
    }
...
int main(int argc, char** argv)
{
  ...
  if (thread_level_provided < MPI_THREAD_MULTIPLE)
    ...
    {
      setup(argv);
      const argp ap = {nullptr,  nullptr, nullptr, argp_program_doc,
      ...
      for (int id = 1; id < size; id++)
        {
          ...
          MPI_Mprobe(id, 0, MPI_COMM_WORLD, &message, &status);
