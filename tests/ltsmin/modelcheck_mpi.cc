...
// ------------------------------------------------------------
  { nullptr, 0, nullptr, 0, "Process options:", 2 },
  {
    ...
    nullptr,
    0,
    "run the SCC computation of Bloemen et al. (PPOPP'16)",
    0 },
  {
    "counterexample",
    'c',
    ...
    0 },
  { "is-empty", 'e', nullptr, 0, "check if the model meets its specification. "
    "Return 1 if a counterexample is found.", 0 },
  { "has-deadlock", 'h', nullptr, 0, "check if the model has a deadlock. "
  ...
  { "parallel", 'p', "INT", 0, "use INT threads (when possible)", 0 },
  {
    ...
    0 },
  { "timer", 't', nullptr, 0, "time the different phases of the execution", 0 },

  {
    ...
    "INT",
    0,
    "use INT MPI processes (when possible)",
    0 },
  { "hostfile", DISTRIBUTED, "STRING", 0, "use STRING file as hostfile", 0 },
...
// ------------------------------------------------------------
  { nullptr, 0, nullptr, 0, "Output options:", 3 },
  {
    ...
    0,
    "output the associated automaton in dot format",
    0 },
  {
    "kripke",
    'k',
    nullptr,
    0,
    "output the associated automaton in (internal) kripke format",
    0 },
  { "csv", CSV, nullptr, 0, "output a CSV containing interesting values", 0 },
// ------------------------------------------------------------
...
{ options, parse_opt_finput, nullptr, nullptr, nullptr, nullptr, nullptr };
...
static std::string
split_filename(const std::string& str)
{
             ...
             struct spot::mpi::attributes_& process_attributes)
{
  ...
  {
    if (mc_options.dead_ap == nullptr || !strcasecmp(mc_options.dead_ap,
                                                     "true"))
  ...
  if (mc_options.model != nullptr)
  {
    ...
    try
    {
      model = spot::ltsmin_model::load(mc_options.model).kripke(
          &ap, dict, deadf, mc_options.compress);
    }
  ...
  if (mc_options.nb_threads == 1 && mc_options.formula != nullptr && mc_options
    .model != nullptr)
  {
    ...
    .model != nullptr)
  {
    ...
    try
    {
      modelcube = spot::ltsmin_model::load(mc_options.model).kripkecube(
          propcube->get_ap(), deadf, mc_options.compress,
          mc_options.nb_threads);
    }
    catch (const std::runtime_error& e)
    {
    ...
    auto res =
      spot::modelcheck<spot::ltsmin_kripkecube_ptr, spot::cspins_state,
          ...
          spot::cspins_state_equal>(modelcube, propcube,
    ...
    for (unsigned i = 0; i < std::get<2>(res).size(); ++i)
    {
      ...
      if (mc_options.csv)
      {
        ...
        std::cout
          ...
          << ','
          << (!std::get<2>(res)[i].is_empty ? "EMPTY," : "NONEMPTY,")
  ...
  if (!mc_options.pseudo_distributed && !mc_options.distributed)
  {
    if (mc_options.has_deadlock && mc_options.model != nullptr)
    {
      ...
      {
        modelcube = spot::ltsmin_model::load(mc_options.model).kripkecube(
            { }, deadf, mc_options.compress, mc_options.nb_threads);
      }
      catch (const std::runtime_error& e)
      {
        std::cerr << e.what() << '\n';
      ...
      auto res = spot::has_deadlock<spot::ltsmin_kripkecube_ptr,
          spot::cspins_state, spot::cspins_iterator, spot::cspins_state_hash,
      ...
      for (unsigned i = 0; i < std::get<1>(res).size(); ++i)
      {
        ...
        if (mc_options.csv)
        {
          ...
          std::cout
            ...
            << ','
            << (std::get<1>(res)[i].has_deadlock ? "DEADLOCK," : "NO-DEADLOCK,")
  ...
  if (mc_options.pseudo_distributed || mc_options.distributed)
  {
    ...
    if (process_attributes.comm_parent == MPI_COMM_NULL)
    {
      ...
      if (!universe_size_flag)
      {
        ...
        process_attributes.ss_out << "This MPI does not support UNIVERSE_SIZE."
          "Using " << mc_options.nb_processes << "process total" << std::endl;
      ...
      else
      {
        ...
        MPI_ARGV_NULL,
                       universe_size - 1, info, 0,
                       MPI_COMM_SELF,
                       &process_attributes.comm_children,
                       MPI_ERRCODES_IGNORE);
      }

      MPI_Intercomm_merge(process_attributes.comm_children, 0,
    ...
    else
    {
      ...
      if (comm_parent_size != 1)
      {
        process_attributes.ss_out
          ...
          << std::endl;
        process_attributes.ss_out
          << "Something is wrong with the parent"
          << std::endl;
      ...
      MPI_Intercomm_merge(process_attributes.comm_parent, 1,
                          &process_attributes.comm_everyone);
    }
    ...
    if (mc_options.has_deadlock && mc_options.model != nullptr)
    {
      ...
      auto res = spot::has_deadlock_mpi<spot::ltsmin_kripkecube_ptr,
          spot::cspins_state, spot::cspins_iterator, spot::cspins_state_hash,
          spot::cspins_state_equal>(modelcube, process_attributes);
      tm.stop("deadlock check");
      memused = spot::memusage() - memused;
...
main(int argc, char** argv)
{
  ...
  if (thread_level_provided < MPI_THREAD_MULTIPLE)
  {
    ...
    process_attributes.ss_out
      << "cannot provide thread level support required ... "
  ...
  else
  {
    setup(argv);
    const argp ap =
    { nullptr, nullptr, nullptr, argp_program_doc, children, nullptr, nullptr };
  ...
  if (process_attributes.comm_everyone != MPI_COMM_NULL)
  {
    ...
    if (rank != 0)
    {
      ...
      MPI_CHAR,
               0, 0, process_attributes.comm_everyone);
    ...
    else
    {
      ...
      for (int i = 1; i < size; i++)
      {
        ...
        MPI_Mrecv(cstr_err, message_size, MPI_CHAR, &message, &status);
  ...
  else
  {
    ...
    else
    {
      ...
      MPI_Send(cstr_err, str_err.size() + 1,
      MPI_CHAR,
               0, 0, process_attributes.comm_parent);
      cstr_out = str_out.c_str();
      MPI_Send(cstr_out, str_out.size() + 1,
      MPI_CHAR,
               0, 0, process_attributes.comm_parent);
