...
template <typename kripke_ptr, typename State, typename Iterator, typename Hash,
          typename Equal>
static std::tuple<bool, std::string, std::vector<istats>> modelcheck(
    kripke_ptr sys, spot::twacube_ptr twa, bool compute_ctrx = false)
{
  ...
  for (unsigned i = 0; i < sys->get_threads(); ++i)
    threads.push_back(std::thread(
        &ec_renault13lpar<State, Iterator, Hash, Equal>::run, &ecs[i]));
...
template <typename kripke_ptr, typename State, typename Iterator, typename Hash,
          typename Equal>
static std::pair<std::vector<bloemen_stats>, spot::timer_map> bloemen(
    kripke_ptr sys)
{
  spot::timer_map tm;
...
template <typename kripke_ptr, typename State, typename Iterator, typename Hash,
          typename Equal>
static std::tuple<bool, std::vector<deadlock_stats>, spot::timer_map>
d_has_deadlock(kripke_ptr sys, spot::process& p)
{
  int rank = 0;
  ...
  if (nbth <= 1)
    {
      ...
      p.ssout << "Process #" << rank << ": on CPU " << proc_name << std::endl;
      swarmed->run();
      tm.start("Run");
      ...
      for (unsigned i = 0; i < nbth; ++i)
        {
          threads[i] = std::thread(
              [&swarmed, &iomutex, i, &barrier, rank, proc_name, &io] {
#if defined(unix) || defined(__unix__) || defined(__unix)
                {
                  std::lock_guard<std::mutex> iolock(iomutex);
                  p.ssout << "Process #" << rank << ": on CPU " << proc_name
                           << std::endl;
                  p.ssout << "Thread #" << i << ": on CPU " << sched_getcpu()
                           << '\n';
                }
#endif
          ...
          if (rc != 0)
            {
              ...
              p.sserr << "Process #" << rank << ": on CPU " << proc_name
                       << std::endl;
              p.sserr << "Error calling pthread_setaffinity_np: " << rc
                       << '\n';
            }
#endif
        }
